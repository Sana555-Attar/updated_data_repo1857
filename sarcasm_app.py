...
model, tokenizer = get_model_And_tokenizer('d82b2b9e8c7a4ddbb18db3ac81a2fd67')

sklearn_model_path = Model(model_id="6473a2118ad5454a9281a11aefa720bb").get_local_copy()
...
def create_clearml_dataset_version(csv_filename, amount, counter):
    ...
    if paths:
        latest_clearml_dataset_id = Dataset.get(dataset_project="tf_sarcasm_detector", dataset_name="tf_sarcasm_dataset").id
        print(f"{latest_clearml_dataset_id=}")
        updated_dataset = Dataset.create(
            dataset_project="tf_sarcasm_detector",
